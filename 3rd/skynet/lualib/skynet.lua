...
-- 与skynet.call()的区别是对于结果没有调p.unpack()
...
-- 这个函数的作用是设置typename类型的消息的dispatch函数为func，这个函数不分发消息
...
-- 是真正的分发消息，对于每个非PTYPE_RESPONSE消息，都会调用co_create()来创建/复用coroutine
...
function skynet.dispatch_message(...)
	-- 先调用raw_dispatch_message分发一条消息
	local succ, err = pcall(raw_dispatch_message,...)

	-- 然后唤醒一遍fork_queue里的所有coroutine
	while true do
